...
#FA vs FT
pfamFAFT<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1:25))
write.table(pfamFAFT,"g-diversity-core-metrics114K/rarefied_table/feature-table-FAFT.txt",sep = "\t",row.names = FALSE, quote=FALSE)

mdFAFT<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="NA")

#just light
pfamFAFTL<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1:7,14:19))
...
mdFAFTL<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="NA" & phase=="light")

#just dark
pfamFAFTD<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1,8:13,20:25))
...
mdFAFTD<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="NA" & phase=="dark")

#FT vs NA
pfamFTNA<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1,14:43))
write.table(pfamFTNA,"g-diversity-core-metrics114K/rarefied_table/feature-table-FTNA.txt",sep = "\t",row.names = FALSE, quote=FALSE)

mdFTNA<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="FA")

#light
pfamFTNAL<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1,14:19,26:34))
...
mdFTNAL<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="FA"& phase=="light")

#dark
pfamFTNAD<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1,20:25,35:43))
...
mdFTNAD<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="FA"& phase=="dark")

#FA vs NA
pfamFANA<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1:13,26:43))
write.table(pfamFANA,"g-diversity-core-metrics114K/rarefied_table/feature-table-FANA.txt",sep = "\t",row.names = FALSE, quote=FALSE)

mdFANA<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="FT")

#light
pfamFANAL<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1:7,26:34))
...
mdFANAL<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="FT"& phase=="light")

#dark
pfamFANAD<-fread("g-diversity-core-metrics114K/rarefied_table/pfam-144k.tsv")%>%dplyr::select(c(1,8:13,35:43))
...
mdFANAD<-fread("metaT_metadata_ztcat_noNT.txt")%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
  filter(condition!="FT"& phase=="dark")

###########################################################

#get the total read amt for PF04563.18 (rpob)
mtb<-fread("pfam/pfam_clean_noNT.tsv")%>%
  filter(FeatureID=="PF04563.18")%>%
  column_to_rownames("FeatureID")%>%
  rowSums() #22K is the depth of RPOB
...
write.table(FAFT.effect,"pfam/aldex/SFR24_0529_FAFT_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
pfamFAFT<-fread("g-diversity-core-metrics114K/rarefied_table/feature-table-FAFTL.txt")%>%column_to_rownames("FeatureID")
...
write.table(FAFTL.effect,"pfam/aldex/SFR24_0529_FAFTL_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FAFTL.effect<-fread("pfam/aldex/SFR24_0529_FAFTL_ald_effectwpval_144K.txt")
...
condsFAvsFT<-conds[1:12]
pfamFAFT<-fread("g-diversity-core-metrics114K/rarefied_table/feature-table-FAFTD.txt")%>%column_to_rownames("FeatureID")
...
write.table(FAFTD.effect,"pfam/aldex/SFR24_0529_FAFTD_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FAFTD.effect<-fread("pfam/aldex/SFR24_0529_FAFTD_ald_effectwpval_144K.txt")
...
write.table(FTNA.effect,"pfam/aldex/SFR24_0529_FTNA_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FTNA.effect<-fread("pfam/aldex/SFR24_0529_FTNA_ald_effectwpval_144K.txt")
...
pfamFTNA<-fread("g-diversity-core-metrics114K/rarefied_table/feature-table-FTNAL.txt")%>%column_to_rownames("FeatureID")
...
FTNAL.effect<-aldex.effect(FTNAL.ald)%>%rownames_to_column("FeatureID")%>%left_join(.,FTNAL.ttest,by="FeatureID")%>%
  mutate(diff.btw=-(diff.btw))
write.table(FTNAL.effect,"pfam/aldex/SFR24_0529_FTNAL_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FTNAL.effect<-fread("pfam/aldex/SFR24_0529_FTNAL_ald_effectwpval_144K.txt")
...
condsFTvsNA<-conds[c(7:21)]
pfamFTNA<-fread("g-diversity-core-metrics114K/rarefied_table/feature-table-FTNAD.txt")%>%column_to_rownames("FeatureID")
...
FTNAD.effect<-aldex.effect(FTNAD.ald)%>%rownames_to_column("FeatureID")%>%left_join(.,FTNAD.ttest,by="FeatureID")%>%
  mutate(diff.btw=-(diff.btw))
write.table(FTNAD.effect,"pfam/aldex/SFR24_0529_FTNAD_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FTNAD.effect<-fread("pfam/aldex/SFR24_0529_FTNAD_ald_effectwpval_144K.txt")
...
FANA.effect<-aldex.effect(FANA.ald)%>%rownames_to_column("FeatureID")%>%left_join(.,FANA.ttest,by="FeatureID")%>%
  mutate(diff.btw=-(diff.btw))
write.table(FANA.effect,"pfam/aldex/SFR24_0529_FANA_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FANA.effect<-fread("pfam/aldex/SFR24_0529_FANA_ald_effectwpval_144K.txt")
...
pfamFANA<-fread("g-diversity-core-metrics114K/rarefied_table/feature-table-FANAL.txt")%>%column_to_rownames("FeatureID")
...
FANAL.effect<-aldex.effect(FANAL.ald)%>%rownames_to_column("FeatureID")%>%left_join(.,FANAL.ttest,by="FeatureID")%>%
  mutate(diff.btw=-(diff.btw))
write.table(FANAL.effect,"pfam/aldex/SFR24_0529_FANAL_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FANAL.effect<-fread("pfam/aldex/SFR24_0529_FANAL_ald_effectwpval_144K.txt")
...
condsFAvsNA<-conds[c(1:6,13:21)]
pfamFANA<-fread("g-diversity-core-metrics114K/rarefied_table/feature-table-FANAD.txt")%>%column_to_rownames("FeatureID")
...
FANAD.effect<-aldex.effect(FANAD.ald)%>%rownames_to_column("FeatureID")%>%left_join(.,FANAD.ttest,by="FeatureID")%>%
  mutate(diff.btw=-(diff.btw))
write.table(FANAD.effect,"pfam/aldex/SFR24_0529_FANAD_ald_effectwpval_144K.txt",sep = "\t",row.names = FALSE, quote=FALSE)

FANAD.effect<-fread("pfam/aldex/SFR24_0529_FANAD_ald_effectwpval_144K.txt")
...
aldex_summhits <- data.frame (comparison  = c("NAFA","NAFA","NAFT","NAFT","FAFT","FAFT"),
                              ...
                              perc= c(13.3,2.77,14.9,3.44,0.545,0.515)) %>%
...
ggsave("pfam/aldex/SFR24_0529_summaldexhits_pless0.05_144K.pdf", width = 2.2, height = 3)
...
aldex_summhitsLD <- data.frame (comparison  = c("NAFA","NAFA","NAFT","NAFT","FAFT","FAFT","NAFA","NAFA","NAFT","NAFT","FAFT","FAFT"),
                              ...
                              perc= c(7.95,1.96,9.93,3.23,0.0648,0.324,6.04,2.50,5.63,2.97,0.798,1.70)) %>%
  mutate(comparison=factor(comparison, levels = c("NAFA","NAFT","FAFT")),
...
ggsave("pfam/aldex/SFR24_0529_summaldexhitsLD_pless0.05_144K.pdf", width = 2.2, height = 3)

#to calc chi-squared

aldex_summhitsLD_stat <- data.frame (comparison  = c("NAFA","NAFT","FAFT","NAFA","NAFT","FAFT"),
                                phase= c("light","light","light","dark","dark","dark"),
                                sig= c(429,567,12,311,310,75),
                                not_sig=c(3897,3743,3076,3333,3297,2933))

#FANA
chi<-aldex_summhitsLD_stat%>%
  filter(comparison=="NAFA")%>%
  column_to_rownames("phase")%>%
  dplyr::select(2:3)%>%
  as.matrix()

chisq.test(chi, simulate.p.value=TRUE, B=2000) #X-squared = 4.4864, df = NA, p-value = 0.03898
...
chisq.test(chi, simulate.p.value=TRUE, B=2000) #X-squared = 41.471, df = NA, p-value = 0.0004998
...
chisq.test(chi, simulate.p.value=TRUE, B=2000) #X-squared = 47.982, df = NA, p-value = 0.0004998
...
#ggsave("aldex/SFR23_0524_venn_pless0.05.pdf", width = 3.5, height = 3)
...
ggsave("aldex/SFR23_0524_summaldexhitsUQ_pless0.05.pdf", width = 2.2, height = 3)
...
ggsave("aldex/SFR23_0524_summaldexhitsLD_unique_pless0.05.pdf", width = 2.2, height = 3)
...
#write.table(unique_hits_all,"aldex/SFR23_0524_FAFT_94uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
#write.table(unique_hits_all,"aldex/SFR23_0524_FTNA_315uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
write.table(unique_hits_all,"aldex/SFR23_0524_FANA_331uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
ggvenn(list_venn, c("FAFT", "FANA", "FTNA"),
       fill_color = c("white","black","gray"))
#ggsave("aldex/SFR23_0524_Light_venn_pless0.05.pdf", width = 3.5, height = 3)
...
#write.table(unique_hits_light,"aldex/SFR23_0524_FAFTL_28uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
#write.table(unique_hits_lightFANA,"aldex/SFR23_0524_FANAL_278uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
write.table(unique_hits_lightFTNA,"aldex/SFR23_0524_FTNAL_424uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
ggvenn(list_venn, c("FAFT", "FANA", "FTNA"),
       fill_color = c("white","black","gray"))
#ggsave("aldex/SFR23_0524_Dark_venn_pless0.05.pdf", width = 3.5, height = 3)
...
#write.table(unique_hits_dark,"aldex/SFR22_0524_FAFTD_156uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
#write.table(unique_hits_darkFANA,"aldex/SFR23_0524_FANAD_431uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
write.table(unique_hits_darkFTNA,"aldex/SFR23_0524_FTNAD_217uniquehits_pless0.05.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
grid.draw(p)
pdf(file="aldex/SFR23_0524_FAFT_LDintersect_pless0.05.pdf")
grid.draw(p)
dev.off()
...
grid.draw(p)
pdf(file="aldex/SFR23_0524_FTNA_LDintersect_pless0.05.pdf")
grid.draw(p)
dev.off()
...
grid.draw(p)
pdf(file="aldex/SFR23_0524_FANA_LDintersect_pless0.05.pdf")
grid.draw(p)
dev.off()
###########################################################
...
ggsave("aldex/SFR23_0524_FAFT_GOterms.pdf",height=4, width=7)
...
ggsave("aldex/SFR23_0525_FANA_GOterms.pdf",height=6.5, width=6.5)
...
ggsave("aldex/SFR23_0524_FANAL_noDUF_volcanoplot_wgoannot_pless0.05.pdf", width = 3.5, height = 3)
...
ggsave("aldex/SFR23_0524_FANAD_noDUF_volcanoplot_wgoannot_pless0.05.pdf", width = 3.5, height = 3)
...
ggsave("aldex/SFR23_0524_FTNA_top2_GOterms.pdf",height=6, width=8)
...
#   left_join(.,all_uniquehits,by="FeatureID") %>%
#   group_by(label_name,condition,phase,sig_level)%>%summarise(mn_TPM=mean(TPM_counts))%>%
...
# 
# sigFAFTLD_mtx<-pfamTPM_all%>%filter(phase=="dark")%>%
...
#   scale_x_discrete(expand = c(0, 0))+ facet_grid(sig_level~phase,scales="free",space="free")+
...
#   mutate(label_name=paste(FeatureID, Name, sep=" "))%>%
#   left_join(.,md,by="sample_name") %>%
#   group_by(label_name,condition,phase)%>%summarise(mn_TPM=mean(TPM_counts))%>%
#   group_by(label_name)%>%mutate(Zscore=scale(mn_TPM))%>%
#   mutate(condition=factor(condition, levels = c("FT", "FA","NA")),
#          phase=factor(phase, levels = c("light","dark")))
# 
# 
# sigFAFTLD_mtx<-pfamTPM_all%>%filter(phase=="light")%>%
#   dplyr::select(label_name,condition,Zscore)%>%spread(condition,Zscore)
# sigFAFTLD_mtx_mtx <- sigFAFTLD_mtx%>%column_to_rownames("label_name")%>%as.matrix()
# sigFAFTLD_mtx.dendro <- as.dendrogram(hclust(d = dist(x = sigFAFTLD_mtx_mtx)))
# sigFAFTLD_mtx_order <- rownames(sigFAFTLD_mtx_mtx)[order.dendrogram(sigFAFTLD_mtx.dendro)]
...
# plt<-ggplot(pfamTPM_all,aes(x=condition, y=label_name)) +theme_classic()+
#   geom_tile(aes(fill=Zscore))+
#   scale_x_discrete(expand = c(0, 0))+ facet_wrap(~phase,scales="free_x")+
#   theme(axis.ticks.y=element_blank(),panel.spacing.x=unit(0, "lines"),axis.text.y = element_text(size = 5))+
#   scale_fill_viridis(option="inferno") + 
#   xlab("condition") + ylab("transcripts")+labs(fill='TPM Z-score')+ggtitle("")
...
p<-ggplot(BSH_TpMpTPM, aes(x=log10(MG_TPM_counts+1), y=log10(MT_TPM_counts+1), 
                       colour=condition,shape=phase)) + geom_point() +
  scale_color_manual(values=c("#0072B2","#D55E00","#009E73"))+
  geom_abline(intercept = 0, slope = 1, color="black", 
              linetype="dashed")+
...
light<-BSH_TpMpTPM%>%filter(phase=="light")
...
dark<-BSH_TpMpTPM%>%filter(phase=="dark")
...
pairwise.wilcox.test(FT$MT_TPM_counts, FT$phase,p.adjust.method="fdr")

# light
# dark 0.7

FA<-BSH_TpMpTPM%>%filter(condition=="FA")
pairwise.wilcox.test(FA$MT_TPM_counts, FA$phase,p.adjust.method="fdr")

# light
# dark 0.1

pNA<-BSH_TpMpTPM%>%filter(condition=="NA")
pairwise.wilcox.test(pNA$MT_TPM_counts, pNA$phase,p.adjust.method="fdr")
...
md<-fread("/mnt/zarrinpar/scratch/sfloresr/metatranscript/metatranscript/trf_cecal_metadata_noNT.tsv")%>%
  filter(condition=="FT") %>%
  #mutate(condition=ifelse(is.na(condition),"NA",condition)) %>%
  mutate(phase=ifelse(zt_time<13,"light","dark")) 
conds <- md$phase
