...
#just light
pfamFAFTL<-fread(pfam_mtb)%>%dplyr::select(c(1:7,14:19))
write.table(pfamFAFTL,paste0(dir,"/pfam_FAFTL.txt"),sep = "\t",row.names = FALSE, quote=FALSE)

#just dark
pfamFAFTD<-fread(pfam_mtb)%>%dplyr::select(c(1,8:13,20:25))
...
#light
pfamFTNAL<-fread(pfam_mtb)%>%dplyr::select(c(1,14:19,26:34))
write.table(pfamFTNAL,paste0(dir,"/pfam_FTNAL.txt"),sep = "\t",row.names = FALSE, quote=FALSE)

#dark
pfamFTNAD<-fread(pfam_mtb)%>%dplyr::select(c(1,20:25,35:43))
...
#light
pfamFANAL<-fread(pfam_mtb)%>%dplyr::select(c(1:7,26:34))
write.table(pfamFANAL,paste0(dir,"/pfam_FANAL.txt"),sep = "\t",row.names = FALSE, quote=FALSE)

#dark
pfamFANAD<-fread(pfam_mtb)%>%dplyr::select(c(1,8:13,35:43))
...
#FAFT
pfamFAFTD<-read.table("/mnt/zarrinpar/scratch/sfloresr/TRF_multiomics/metatranscript/woltka2_results/pfam-to-go-process.map",header = FALSE, sep = "\t",
...
pfamFAFTL<-read.table("/mnt/zarrinpar/scratch/sfloresr/TRF_multiomics/metatranscript/woltka2_results/pfam-to-go-process.map",header = FALSE, sep = "\t",
                      col.names = paste0("V",seq_len(4)), fill = TRUE)%>%
  gather(column,GO_Term,-V1)%>%
  dplyr::select(1,3)%>%
  dplyr::rename(FeatureID=V1)%>%
...
pfamFANAL<-read.table("/mnt/zarrinpar/scratch/sfloresr/TRF_multiomics/metatranscript/woltka2_results/pfam-to-go-process.map",header = FALSE, sep = "\t",
                      col.names = paste0("V",seq_len(4)), fill = TRUE)%>%
  gather(column,GO_Term,-V1)%>%
  dplyr::select(1,3)%>%
  dplyr::rename(FeatureID=V1)%>%
...
ggsave(paste0(dir2,"FANA_GOterms.pdf"),height=6.5, width=8)

#FTNA
pfamFTNAD<-read.table("/mnt/zarrinpar/scratch/sfloresr/TRF_multiomics/metatranscript/woltka2_results/pfam-to-go-process.map",header = FALSE, sep = "\t",
                      col.names = paste0("V",seq_len(4)), fill = TRUE)%>%
  gather(column,GO_Term,-V1)%>%
  dplyr::select(1,3)%>%
  dplyr::rename(FeatureID=V1)%>%
...
pfamFTNAL<-read.table("/mnt/zarrinpar/scratch/sfloresr/TRF_multiomics/metatranscript/woltka2_results/pfam-to-go-process.map",header = FALSE, sep = "\t",
                      col.names = paste0("V",seq_len(4)), fill = TRUE)%>%
  gather(column,GO_Term,-V1)%>%
  dplyr::select(1,3)%>%
  dplyr::rename(FeatureID=V1)%>%
...
ggsave(paste0(dir2,"FTNA_GOterms.pdf"),height=8, width=8)
...
ZT_dist <- function(data,FeatureID) {
  ...
  ggsave(paste("pfam/aldex/FAFTD_zt/",FeatureID,".pdf",sep=""), height=3, width=5)
}
