...
FT_metacyc<-fread("cyclic_analysis/wol2_dna_pipeline/FT_metacycle/meta2d_filtered_dna_FT.txt")%>%
...
#read in FT metacycle hits
FT_metacyc<-fread("cyclic_analysis/wol2_dna_pipeline_rpob/FT_metacycle/meta2d_filtered_dna_FT.txt")%>%
  dplyr::rename(FeatureID=CycID) %>%
  left_join(.,pfam_annot, by="FeatureID")%>%
  arrange(JTK_adjphase)%>%
  mutate(label_name=paste(FeatureID, Name, sep=" "))
...
#read in FA metacycle hits
FA_metacyc<-fread("cyclic_analysis/wol2_dna_pipeline_rpob/FA_metacycle/meta2d_filtered_dna_FA.txt")%>%
  dplyr::rename(FeatureID=CycID) %>%
  left_join(.,pfam_annot, by="FeatureID")%>%
  arrange(JTK_adjphase)%>%
  mutate(label_name=paste(FeatureID, Name, sep=" "))
...
#read in NA metacycle hits
NA_metacyc<-fread("cyclic_analysis/wol2_dna_pipeline_rpob/NA_metacycle/meta2d_filtered_dna_NA.txt")%>%
  dplyr::rename(FeatureID=CycID) %>%
  left_join(.,pfam_annot, by="FeatureID")%>%
  arrange(JTK_adjphase)%>%
  mutate(label_name=paste(FeatureID, Name, sep=" "))
...
cyc_summ<-data.frame(condition=c("NA","FA","FT"),
                     cycling_n=c(308,42,45),
                     not_cycling_n=c(3473-308,3473-42,3473-45))%>%
  mutate(cycling_perc=(cycling_n/3473)*100,
         not_cycling_perc=(not_cycling_n/3473)*100)
write.table(cyc_summ,"cyclic_analysis/wol2_dna_pipeline_rpob/SFR23_0619_MG_cycling_summary_table.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
chisq.test(mtx_chi,simulate.p.value=TRUE, B=2000) #X-squared = 368.22, df = NA, p-value = 0.0004998 diff among 3 conditions
...
ggsave("cyclic_analysis/wol2_dna_pipeline_rpob/SFR23_0619_MG_cycling_summ_barplot.pdf", plot=p,height=3, width=2.5)
...
draw.venn(sigNA$FeatureID,sigFA$FeatureID,sigFT$FeatureID,
          ...
          output="pdf",
          filename="cyclic_analysis/wol2_dna_pipeline_rpob/SFR23_0619_MG_venn_overlap.pdf")
