...
##############################################################################
#paths
notnorm_data<-"data/bsh_analysis/BSH_proteindb_metaT/genome.tsv"
norm_data<-"data/bsh_analysis/BSH_proteindb_metaT/genome-TPM.tsv"
bdm_FAFT_light<-"data/bsh_analysis/BSH_proteindb_metaT/birdman_outputs/genomeL_noNT_rmdbton.beta_var.tsv"
bdm_FAFT_dark<-"data/bsh_analysis/BSH_proteindb_metaT/birdman_outputs/genomeD_noNT_rmdbton.beta_var.tsv"
...
write.table(dat_nn,paste0(dat_path,"BSH_proteindb_metaT/genome_noNT.tsv"),sep = "\t",row.names = FALSE, quote=FALSE)

dat<-dat_nn%>%column_to_rownames("FeatureID")
dat_rmzs <- dat[!(rowSums(dat != 0) <2), ]
dat_rmzs<-dat_rmzs%>%rownames_to_column("FeatureID")
write.table(dat_rmzs,paste0(dat_path,"BSH_proteindb_metaT/genome_noNT_rmdbton.tsv"),sep = "\t",row.names = FALSE, quote=FALSE)

dat_n<-subset_dat(norm_data)
write.table(dat_n,paste0(dat_path,"BSH_proteindb_metaT/genome-TPM_noNT.tsv"),sep = "\t",row.names = FALSE, quote=FALSE)
###########################################################
#subset by light and dark-->to run birdman

#just light
dat_rmzs_l<-subset_dat_bshLD(dat_rmzs,"light")
write.table(dat_rmzs_l,paste0("BSH_proteindb_metaT/genomeL_noNT_rmdbton.tsv"),sep = "\t",row.names = FALSE, quote=FALSE)

#just dark
dat_rmzs_d<-subset_dat_bshLD(dat_rmzs,"dark")
write.table(dat_rmzs_d,"BSH_proteindb_metaT/genomeD_noNT_rmdbton.tsv",sep = "\t",row.names = FALSE, quote=FALSE)
...
natlog_bsh<-fread(paste0(dat_path,"BSH_proteindb_metaT/rpca_results_genome/sample_plot_data_10tp.tsv"))%>%
  dplyr::select(1:2)%>%
...
write.table(BSH,paste0(dat_path,"BSH_proteindb_metaT/birdman_outputs/genomeLD_noNT_rmdbton.beta_var.tsv"),sep = "\t",row.names = FALSE, quote=FALSE)
