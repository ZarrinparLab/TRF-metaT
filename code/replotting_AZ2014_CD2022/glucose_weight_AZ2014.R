...
ggsave(paste0(res,"avg_mouse_weight.pdf"), plot=weights_plot,height=3.2, width=3)

for (i in 0:8){
  weight_w_test <- pairwise.wilcox.test(x= subset(wgt_df, week==i)$weight, 
...
#################################################################
#plot mouse glucose--Figure S1B

glucose_df<-fread(input_glucose)%>%
  mutate(condition=ifelse(is.na(condition),"NA",condition))%>%
...
ggsave(paste0(res,"mouse_glucose.pdf"), plot=glucose_plot,height=3.2, width=3)

time_list<-unique(glucose_df$time)
for (i in 1:5){
  
  glucose_w_test <- pairwise.wilcox.test(x= subset(glucose_df, time==time_list[i])$glucose, 
                                        g=subset(glucose_df, time==time_list[i])$condition,
                                        p.adjust.method = "fdr")
  print(time_list[i])
  print(glucose_w_test)
}
